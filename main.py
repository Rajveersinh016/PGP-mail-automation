...
→ 4. Gemini AI Analysis → 5. Report (Simplified) → 6. Email → 7. Cleanup
"""
...
def load_watchlist_keywords() -> list[str]:
    """Parse watchlist.json and extract lowercase company names and shorthand tokens."""
    ...
    try:
        ...
        keywords = set()
        for cat, companies in watchlist.items():
            for c in companies:
                c_clean = c.strip()
                keywords.add(c_clean.lower())
                
                # Shorthand: strip common corporate suffixes to get core brand name
                core = re.sub(r'\b(Glass|Pharma|Group|Holdings|Industries|Distilleries)\b', '', c_clean, flags=re.IGNORECASE).strip()
                if len(core) >= 3:
                    keywords.add(core.lower())
                if c_clean == "HNG":
                    keywords.add("hindustan national glass")
...
def local_pre_filter(articles: list[dict], watchlist_keys: list[str]) -> list[dict]:
    """
    Keep only articles published in the last 24h that contain 
    at least one watchlist company AND at least one container glass context term.
    """
    filtered = []
    ...
    company_patterns = [re.compile(rf"\b{re.escape(k)}\b", re.IGNORECASE) for k in watchlist_keys]
    context_patterns = [re.compile(rf"\b{re.escape(w)}\b", re.IGNORECASE) for w in CONTEXT_KEYWORDS]

    log.info(f"Applying local filter (24h limit, {len(watchlist_keys)} company keys, {len(CONTEXT_KEYWORDS)} context keywords)...")

    for a in articles:
        # 1. Date Check (strict 24 hours)
        ...
        if pub_str:
            try:
                ...
                pass # If parsing fails, proceed to text validation
                
        # 2. Company & Context Checks in title or text
...
def main():
    ...
    _banner("PGP Container Glass Daily Intelligence Pipeline")
    log.info(f"Started: {start_time.strftime('%Y-%m-%d %H:%M:%S UTC')}")

    # Verify key environment variables
    if not os.environ.get("GEMINI_API_KEY"):
        log.error("✗ GEMINI_API_KEY is missing. Pipeline cannot continue.")
        sys.exit(1)
    ...
    watchlist_keys = load_watchlist_keywords()
    filtered = local_pre_filter(raw_articles, watchlist_keys)
    ...
    new_articles = get_new_articles(filtered)
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if analysis:
                    ...
                    article["business_impact"] = analysis.get("business_impact", "")
                    
                    curated_articles.append(article)
                    log.info(f"    ✓ RELEVANT: {article['title'][:55]} -> Company={article['company']} | Category={article['category']}")
                else:
    ...
    html_body, excel_path = generate_report(curated_articles)
